...
public class Astar {
	...
	public static int[][] calcMoverHeuristic(int[][] map)
	{
		...
		for(int i = 0; i < goals.size(); i += 2){
			ret[goals.get(i)][goals.get(i+1)] = 0;
			calcMover(ret,map,goals.get(i),goals.get(i+1));
		}
		
		return ret;
	}

	private static void calcMover(int[][] heuristic, int[][] map, int startx, int starty)
	{
		// up
		if (map[startx][starty-1] > 1 && map[startx][starty-1] < 9999)
			if(heuristic[startx][starty-1] > heuristic[startx][starty]+1)
			{
				heuristic[startx][starty-1] = heuristic[startx][starty]+1;
				calcMover(heuristic,map,startx,starty-1);
			}
		// down
		if (map[startx][starty+1] > 1 && map[startx][starty+1] < 9999)
			if(heuristic[startx][starty+1] > heuristic[startx][starty]+1)
		...
		// left
		if (map[startx-1][starty] > 1 && map[startx-1][starty] < 9999)
			if(heuristic[startx-1][starty] > heuristic[startx][starty]+1)
			{
				heuristic[startx-1][starty] = heuristic[startx][starty]+1;
				calcMover(heuristic,map,startx-1,starty);
			}
		// right
		if (map[startx+1][starty] > 1 && map[startx+1][starty] < 9999)
			if(heuristic[startx+1][starty] > heuristic[startx][starty]+1)
	...
	private static void calcBox(int[][] heuristic, int[][] map, int startx, int starty)
	{
		// up
		if (map[startx][starty-1] == 2 )
			if(heuristic[startx][starty-1] > heuristic[startx][starty]+1)
			{
				heuristic[startx][starty-1] = heuristic[startx][starty]+1;
				calcBox(heuristic,map,startx,starty-1);
			}
		// down
		if (map[startx][starty+1] == 2 )
			if(heuristic[startx][starty+1] > heuristic[startx][starty]+1)
		...
		// left
		if (map[startx-1][starty] == 2 )
			if(heuristic[startx-1][starty] > heuristic[startx][starty]+1)
			{
				heuristic[startx-1][starty] = heuristic[startx][starty]+1;
				calcBox(heuristic,map,startx-1,starty);
			}
		// right
		if (map[startx+1][starty] == 2 )
			if(heuristic[startx+1][starty] > heuristic[startx][starty]+1)
	...
	public static int[][] calcBoxHeuristic(int[][] map)
	{
		...
		for(int i = 0; i < goals.size(); i += 2){
			ret[goals.get(i)][goals.get(i+1)] = 0;
			calcBox(ret,map,goals.get(i),goals.get(i+1));
		}
		
		return ret;
	}
	
	public static int[][] calcBoxHeuristicOld(int[][] map)
	{
		int[][] ret = new int[map.length][map[0].length];
		Square[][] squares = new Square[map.length][map[0].length];
